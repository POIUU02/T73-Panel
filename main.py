...
CONFIG = {
    ...
    "secret": os.environ.get("SECRET_KEY", "vroom-default-secret-key"),
}
...
<script>
let lang=localStorage.getItem('vroom_lang')||'en';
let theme=localStorage.getItem('vroom_theme')||'dark';
function setLang(l){lang=l;document.body.dir=l==='fa'?'rtl':'ltr';document.querySelectorAll('[data-en]').forEach(el=>{const v=el.getAttribute('data-'+l);if(v)el.textContent=v});document.getElementById('lang-toggle').textContent=l.toUpperCase();localStorage.setItem('vroom_lang',l)}
...
<div class="mobile-header">
  <span style="font-weight:700;font-size:13px">VROOM</span>
...
function downloadQR(){const img=$('#qr-img');if(!img.src)return;const a=document.createElement('a');a.href=img.src;a.download='vroom-qr.png';a.click()}
